...
# `/courseware-collab` 技能

## 1. 边界

- 只消费 `content_type: lesson` 且 `review_status: human_approved` 的教学设计。
- 只生成 `输出/{课时名}_课件.md` 与 `输出/{课时名}_课堂提问参考答案.md`。
- 不重新生成学习目标、评价任务、活动设计或教学设计。
- 图片只允许引用输出文件同级 `./images/{文件名}`，禁止 Markdown 图片语法。
- 学生姓名只来自 `学生近期数据/学生成绩数据.md`，禁止虚构。
...
1. `AGENTS.md`
2. `主控/技能调用协议.md`
3. `主控/工作流注册表.md` 的 `/courseware-collab`
4. `主控/输出合约.md`
5. `主控/图片引用协议.md`
6. `主控/审核协议.md`
7. `知识库/教材原文/教材原文_教材课时分配.md`
8. 对应课时的教学设计、教材原文、练习册题库

### 确认门1前读取

- `技能/课件设计技能/技能.md` 与 `技能/课件设计技能/检查清单.md`
- 对应教材原文中所有支撑作答的材料：实际问题背景、原始数据、教材步骤、统计表/方案表、教材图片、练习题干或题干摘要

### 确认门2前读取

- `技能/提问质量技能/技能.md` 与 `技能/提问质量技能/检查清单.md`

### 确认门3前读取

- `技能/分层提问技能/技能.md` 与 `技能/分层提问技能/检查清单.md`
- 学生成绩数据、提问历史记录

### 生成与验证前读取

- `技能/参考答案生成技能/技能.md` 与 `技能/参考答案生成技能/检查清单.md`
- `技能/图片资源技能/技能.md`
- `验证器/参考答案验证器/验证规则.md`
- `验证器/课件验证器/验证规则.md`
- `验证器/图片资源验证器/验证规则.md`
...
1. 课题确认：匹配课时，检查教学设计存在且为 `human_approved`，显示章节、节次、课时、文件、状态，等待教师确认。
...
3. 确认门2：课堂提问设计。呈现前必须按提问质量技能自检；低质量、冗余或脱节问题必须先删除、合并或改写。每个问题必须标注“材料锚点页”，证明学生在课件内可见材料基础上可以作答；缺少材料锚点的问题必须回退到确认门1补页或补材料。
...
每个确认门使用主控确认门模板，并列出 3-5 条“需要您确认的关键点”。

确认门2的表格必须包含：

| 编号 | 页面序号 | 材料锚点页 | 对应教学设计 | 问题内容 | 提示/追问 | 指向后续结论 | 提问层级 | 产出形式 | 质量自检 |
|---|---:|---|---|---|---|---|---|---|---|
...
```powershell
python tools/validate_output.py 输出/{课时名}_课件.md --lesson-file 输出/{课时名}_教学设计.md --question-reference 输出/{课时名}_课堂提问参考答案.md --students-file 学生近期数据/学生成绩数据.md
```
...
- 新增或迁移的图片必须复制到 `输出/images/`，最终课件只引用 `./images/{文件名}`。
