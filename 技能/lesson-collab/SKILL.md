...
- 教学目标、评价设计和活动设计必须读取 `学生近期数据/学生成绩数据.md`。
- `学生近期数据/示例.md` 与 `学生近期数据/模板.md` 仅用于格式说明，不得替代真实学生成绩数据。
...
| 项目 | 内容 |
|------|------|
| 负责技能 | `技能/作业设计技能/技能.md` |
| 执行方式 | AI 自动完成 |
| 输入 | 学习目标 + 评价任务 + 知识分析 + `知识库/练习册题库/` + 对应课时教材参考解答 |
...
- 时间验证“有条件通过”时，第一层课堂实施要点必须写明可后移的 `flex` 或 `backup` 活动；“不通过”时不得输出教学设计。
...
`question_id` 与 `question_ids` 二选一。禁止用逗号拼接多个编号；禁止为每个编号重复输出相同的 `source_id`、`source_type`。
