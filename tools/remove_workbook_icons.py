"""Remove section icon images from workbook markdown files.

Finds image references immediately preceding target section headers
(one-level `# 栏目名`), removes the reference lines from the MD files,
and deletes the actual image files under knowledge/images/.

This is the workbook counterpart of tools/remove_section_icons.py.
Key differences from the textbook version:
  - Section headers are one-level (`# `) not two-level (`## `).
  - Workbook sections: 知识点拨 / 夯实基础 / 数学思考 / 解决问题 / 开阔视野.
  - Icons appear selectively (only before some headers), so the tool
    matches the pattern unconditionally — a header without a preceding
    image is simply never matched.

Usage:
    python tools/remove_workbook_icons.py <target_dir> [--headers 栏目1,栏目2,...] [--dry-run] [--no-delete-images]
"""
...
def _build_icon_header_pattern(headers):
    ...
    Each header may carry a leading noise glyph (e.g. a full-width '?')
    introduced by OCR, so we allow optional non-heading characters before
    the canonical name. The header text itself is never modified.
    """
    parts = []
    for h in headers:
        # allow optional leading noise chars (non-'#', non-whitespace) before name
        parts.append(r"[^\s#]*" + re.escape(h))
    return "|".join(parts)
...
def main():
    parser = argparse.ArgumentParser(
        description="Remove section icon images from workbook markdown files."
    )
    parser.add_argument(
        "target_dir",
        help="Directory containing workbook MD files (e.g. knowledge/workbooks)",
    )
    parser.add_argument(
        "--headers",
        help="Comma-separated list of target section headers (default: all 5)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
