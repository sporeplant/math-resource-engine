...
教学设计、课件、课堂提问参考答案和复习讲义写入 `输出/`；教材问题参考解答作为知识库资产写入 `知识库/教材参考解答/`：
...
| 复习讲义 | `输出/八下复习讲义/{讲义编号}_{讲义名称}_复习讲义.md` |
...
该知识库产物不设置 `review_status`。正常题目不设置状态字段。
...
- `/lesson-collab`、`/courseware-collab` 产物必须在 `source_files` 中登记对应课时的 `知识库/教材参考解答/教材参考解答_{课时编号}_{课时名称}.md`；缺失时不得生成产物。
- `/复习讲义` 产物必须在 `source_files` 中登记所有被整理的 `知识库/复习讲义/` 源文件。
...
| textbook_solution | `/教材问题解答` | 教材全部问题的参考解答知识库 |
...
`question_id` 与 `question_ids` 二选一。禁止把多个编号写入一个逗号分隔的 `question_id` 字符串，也禁止重复输出相同的 `source_id`、`source_type`。
...
| 第二层：完整结构化设计 | 审核者、验证器、课件生成流程 | 教学评审核、题源追踪、下游复用 | 保留目标、评价、问题链、活动、题源、作业、一致性矩阵和质量记录 |

### 8.1 第一层：课堂实施导航

一级标题之后先给出阅读建议，再按以下固定顺序输出：
...
`lesson_flow` 中每个活动还必须包含 `time_priority` 和完整 `time_budget`。教学设计输出前必须运行教学活动时间验证器；不通过时回退活动设计，有条件通过时在课堂实施要点中明确可后移任务。
