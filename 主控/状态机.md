...
系统不得自动把 `pending_human_review` 改为 `human_approved`。
...
| 状态 | 说明 |
|------|------|
| COURSEWARE_INIT | 接收 `/courseware` 输入，读取规则 |
| APPROVED_LESSON_LOAD | 读取人工审核通过的教学设计 |
| COURSEWARE_PRECHECK | 检查 `review_status: human_approved` |
...
- `/lesson` 到达 `LESSON_PENDING_REVIEW`。
- `/courseware` 到达 `COURSEWARE_COMPLETE`。

强制停止：

- `/courseware` 读取到的教学设计不是 `human_approved`。
- 任一硬性质量门不通过。
- 同一节点连续 3 次不通过。
- 用户主动中断。
