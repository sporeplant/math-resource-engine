## 第 09 讲专题 4 平行（特殊）四边形中的最值问题

1．如图，在 Rt\(\triangle ABC\) 中， \(\angle C = 90^{\circ}\) ， \(A C = 6\) ，BC＝8，点 N 是 BC 边上一点，点 M 为 AB 边上的动点，点 D、E 分别为 CN，MN 的中点，则 DE 的最小值是（ ）

![](../images/bdb6a7bbc567050c01e98de8bc24c183e5e0cb9ffe005cad51e465f77fcd1c12.jpg)

A．2

B． \(\frac { 12 } { 5 }\)

C．3

D． \(\frac { 24 } { 5 }\)

【解答】解：连接 CM，当 \(C M \bot A B\) 时，CM 的值最小（垂线段最短），此时 DE 有最小值，

![](../images/a4042f8e06bbab2fb77b54d05e919b4691f5c6410ce57629d72937ef1d10fa1a.jpg)

理由是： \(\because \angle C = 90^{\circ}\) ， \(A C = 6\) ，BC＝8，


$\(\therefore A B = \sqrt {A C^{2} + B C^{2}} = \sqrt {6^{2} + 8^{2}} = 10, $\)



$\(\therefore \frac {1}{2} A C \cdot B C = \frac {1}{2} A B \cdot C M, $\)



$\(\therefore \frac {1}{2} \times 6 \times 8 = \frac {1}{2} \times 10 \times C M, $\)



$\(\therefore C M = \frac {24}{5}, $\)


∵点 D、E 分别为 CN，MN 的中点，


$\(\therefore D E = \frac {1}{2} C M = \frac {1}{2} \times \frac {24}{5} = \frac {12}{5}, $\)


即 DE 的最小值是 \(\frac { 12 } { 5 }\)

故选：B．

2．如图，在▱ ABCD 中， \(\angle C = 120^{\circ}\) °， \(\scriptstyle A D = 2 A B = 8\) ，点 H，G 分别是边 CD，BC 上的动点，连接 AH，HG，点 E 为 AH 的中点，点 F 为 GH 的中点，连接 EF，则 EF 的最大值与最小值的差为 \(\sqrt { 3 }\)

![](../images/57194b63d48a3fc276373caebcb000b887c9a351daa46b80bd22a273e64b1772.jpg)

【解答】解：如图：取 AD 的中点 M，连接 CM、AG、AC，过点 A 作 \(A N \bot B C\) 于点 N，

![](../images/89f0cd0fbd9a2dcef08e99893593705e918789184b06a68b0c708e63ff2162dc.jpg)


$\(\therefore A M = D M = \frac {1}{2} A D = \frac {1}{2} \times 8 = 4, $\)


∵四边形 ABCD 是平行四边形， \(\angle B C D = 120^{\circ}, A D = 2 A B = 8\) ，


$\(\therefore \angle D = 180^{\circ} - \angle B C D = 60^{\circ}, A B = C D = \frac {1}{2} A D = \frac {1}{2} \times 8 = 4, $\)



$\(\therefore A M = D M = D C = 4, $\)


\(\therefore \triangle C D M\) 是等边三角形，


$\(\therefore \angle D M C = \angle M C D = 60^{\circ}, A M = M C, $\)



$\(\therefore \angle M A C = \angle M C A = \frac {1}{2} \angle D M C = \frac {1}{2} \times 60^{\circ} = 30^{\circ}, $\)



$\(\therefore \angle A C D = \angle M C A + \angle M C D = 30^{\circ} + 60^{\circ} = 90^{\circ}, $\)


在 \(\mathrm { R t } \triangle A C D\) 中，由勾股定理得： \(A C = \sqrt { A D^{2} - C D^{2} } = \sqrt { 8^{2} - 4^{2} } = 4 \sqrt { 3 }\)

在 \(\mathrm { R t } \triangle A C N\) 中， \(\angle A C N = \angle B C D - \angle A C D = 120^{\circ} \quad - 90^{\circ} = 30^{\circ}\)


$\(\therefore A N = \frac {1}{2} A C = \frac {1}{2} \times 4 \sqrt {3} = 2 \sqrt {3}, $\)



$\(\because A E = E H, \quad G F = F H, $\)


\(\therefore E F\) 是 \(\triangle A H G\) 的中位线，


$\(\therefore E F = \frac {1}{2} A G, $\)


\(\because A G\) 的最大值为 AC 的长，最小值为 AN 的长，

\(\therefore A G\) 的最大值为 \(4 \sqrt { 3 }\) ，最小值为 \(2 \sqrt { 3 }\)

\(\therefore E F\) 的最大值为 \(2 \sqrt { 3 }\) ，最小值为 \(\sqrt { 3 }\)

\(\therefore E F\) 的最大值与最小值的差为 \(2 { \sqrt { 3 } } - { \sqrt { 3 } } = { \sqrt { 3 } }\) ，

故答案为： \(\sqrt { 3 }\)

3．如图，在▱ ABCD 中，已知 \(A B = 4, \ B C = 6, \ C A B C = 60^{\circ}\) °，点 P 是 BC 边上一动点（点 P 不与 B，C 重

合），连接 AP，作点 B 关于直线 AP 的对称点 Q，则线段 QC 的最小值为 \(2 \sqrt { 7 } - 4\)

![](../images/997b62f7372ce5620aeead63c933a07468039acaad21b20e7bab1c03a58d166a.jpg)

【解答】解：如图，过点 A 作 \(A H \bot B C\) 于 H，连接 AC，

![](../images/e56aaea5c5e06f7ec6e58efa75a107de611d993dbdb547c0e380a8f37632ff0b.jpg)

图3


$\(\because A B = 4, B C = 6, \angle A B C = 60^{\circ}, $\)


则 \(A H = A B^{\bullet} \sin \angle A B C = 4 \sin 60^{\circ} \ = 2 { \sqrt { 3 } }, \ B H = A B^{\bullet} \cos \angle A B C = 4 \cos 60^{\circ} \ = 2 { \sqrt { 3 } }, \ B H = 4 \ldots 60^{\circ}\)


$\(\therefore C H = B C - B H = 6 - 2 = 4, $\)


在 \(\mathrm { R t } \triangle A C H\) 中， \(A C = \sqrt { \mathrm { A H }^{2} + \mathrm { C H }^{2} } = \sqrt { ( 2 \sqrt { 3 } )^{2} + 4^{2} } = 2 \sqrt { 7 },\)

∵点 B 与点 Q 关于直线 AP 对称，


$\(\therefore A Q = A B = 4, $\)


∴点 Q 在以 A 为圆心 AB 为半径的 A 上，

∴当 C、Q、A 三点共线时 QC 最小，QC 的最小值＝ \(A C - A Q = 2 \sqrt { 7 } - 4\)

故答案为： \(2 \sqrt { 7 } - 4\)

4．如图，在\(\triangle ABC\) 中， \(\angle B A C = 30^{\circ}\) °， \(A B = A C = 12\) ，P 为 AB 边上一动点，以 PA，PC 为边作平行四边形\(P A Q C\) ，则对角线 PQ 的长度的最小值为 6

![](../images/f18611177a5caea133e90589ec9d34875195e5dc2ddbb91fa0fb147df7662797.jpg)

【解答】解：如图所示：

![](../images/2544643fc81fefec0947625dbdf59b42911013d3246b801ff8235bdc47f38926.jpg)

∵四边形 PAQC 是平行四边形，


$\(\therefore A O = C O, \quad O P = O Q, $\)


∵PQ 最短也就是 PO 最短，

过点 O 作 \(O E \bot A B\) ，当点 P 与 E 重合时， \(O P\) 最短，OE 即为所求，


$\(\because \angle B A C = 30^{\circ}, $\)



$\(\therefore O E = \frac {1}{2} O A, $\)



$\(\because A B = A C = 12, $\)



$\(\because A O = \frac {1}{2} A C = \frac {1}{2} \times 12 = 6, $\)



$\(\therefore O E = 3, $\)


\(\therefore P Q\) 的最小值 \(= 2 O E = 6\) ，

故答案为：6

5．如图，在\(\triangle ABC\) 中，AB＝BC＝10，AC＝12，点 D，E 分别是 AB，BC 边上的动点，连结 DE，F，M分别是 AD，DE 的中点，则 FM 的最小值为（ ）

![](../images/9e0e9546a87a8ca398cd69c03ab0afb32378b600e9a44d4d91d4c4ec44a0784b.jpg)

A．12

B．10

C．9.6

D．4.8

【解答】解：如图，过点 B 作 \(B H \bot A C\) 于 H，

![](../images/527895697cf1b03a8d62082b9d85018efa561c325b93c992b0ae947cdc58028b.jpg)

∵F，M分别是 AD，DE 的中点，


$\(\therefore F M = \frac {1}{2} \mathrm{AE}, $\)


∴当 AE 取最小值时，FM 的值最小，

由垂线段最短可知，当 \(AE \bot BC\) 于点 E 时，AE 的值最小，

在 \(\triangle A B C\) 中， \(\scriptstyle A B = B C = 10, A C = 12\) ，


$\(\therefore C H = \frac {1}{2} A C = 6, $\)



$\(\therefore B H = \sqrt {B C^{2} - C H^{2}} = \sqrt {10^{2} - 6^{2}} = 8, $\)



$\(\therefore S_{\triangle A B C} = \frac {1}{2} \times 12 \times 8 = 48, $\)


又∵ \(S_{\Delta A B C} = \frac { 1 } { 2 } \times B C \times A E\) XBCXAE，


$\(\therefore \frac {1}{2} \times 10 \times A E = 48, $\)



$\(\therefore A E = 9. 6, $\)



$\(\therefore F M = 4. 8, $\)


故选：D．

6．如图，在 \(\triangle A B C\) 中， \(\angle B A C = 30^{\circ}\) ， \(\scriptstyle A B = A C = 4\) ，P 为 AB 边上一动点，以 PA，PC 为邻边作平行四边形 \(P A Q C ;\) ，则对角线 PQ 的最小值为（ ）

![](../images/c7cbb9530406e7fee28270fe56ada1612e0ad1bffd8674c4e34e6b863ad869ab.jpg)

A．2cm

B．2.5cm

C．3cm

D．4cm

【解答】解：如图，过点 C 作 \(C D \bot A B\) 于点 D，

![](../images/a17cc7eebb5fb14760a0f02f8804bff19b8e253d18e08031f7026c06f01f2cc0.jpg)

∵在 \(\mathrm { R t } \triangle A C D\) 中， \(\angle A D C = 90^{\circ}\) ， \(\angle B A C = 30^{\circ}\) °， \(A C = 4 c m\)


$\(\therefore C D = \frac {1}{2} A C = 2 c m, $\)


∵四边形 PAQC 是平行四边形，


$\(\therefore A B \parallel C Q, $\)


∴当 \(P Q \bot A B\) 时，PQ 取得最小值，此时 \(P Q = C D = 2 c m\) ，

故选：A．

7．如图，四边形 ABCD 中， \(\angle A = 90^{\circ}\) ，AB＝8，AD＝6，点 M，N 分别为线段 BC，AB 上的动点（含端点，但点 M 不与点 B 重合），点 E，F 分别为 DM，MN 的中点，则 EF 长度的最大值为（ ）

![](../images/2d71b8ae475a65fabe294553743436b7b6778b23c5f9def35ebcd2131f056060.jpg)

A．4

B．5

C．6

D．10

【解答】解：∵点 E，F 分别为 DM，MN 的中点，

\(\therefore E F\) 是 \(\triangle M N D\) 的中位线，


$\(\therefore E F = \frac {1}{2} D N, $\)


当点 N 与点 B 重合时，DN 最大，此时 \(D N = \sqrt { \ A B^{2} + A D^{2} } = \sqrt { 6^{2} + 8^{2} } = 10\) ，

\(\therefore E F\) 长度的最大值为 5，

故选：B．

8．如图，已知菱形 ABCD 的边长为 6，点 M 是对角线 AC 上的一动点，且 \(\angle A B C = 120^{\circ}\) °，则 \(M A { + } M B { + } M D\) 的最小值是（ ）

![](../images/0ccf608ff342b571881381a88d1b002ec1c66444e1947658a4561f89ef6e1eb6.jpg)

A． \(3 \sqrt { 3 }\)

B． \(3 { + } 3 \sqrt { 3 }\)

C． \(6 { + } \sqrt { 3 }\)

D． \(\mathfrak { G } \sqrt { 3 }\)

【解答】解：如图，过点 D 作 \(D E \bot A B\) 于点 E，连接 \(B D\) ，

![](../images/d37487769676493b562d28d7a482ef694c130db65f8ea1158bcfd1b150a7391a.jpg)

∵菱形 ABCD 中， \(\angle A B C = 120^{\circ}\)


$\(\therefore \angle D A B = 60^{\circ}, A D = A B = D C = B C, $\)


∴\(\triangle ADB\) 是等边三角形，


$\(\therefore \angle M A E = 30^{\circ}, $\)



$\(\therefore A M = 2 M E, $\)



$\(\because M D = M B, $\)



$\(\therefore M A + M B + M D = 2 M E + 2 D M = 2 D E, $\)


根据垂线段最短，此时 DE 最短，即 MA+MB+MD 最小，

∵菱形 ABCD 的边长为 6，


$\(\therefore D E = \sqrt {A D^{2} - A E^{2}} = \sqrt {6^{2} - 3^{2}} = 3 \sqrt {3}, $\)



$\(\therefore 2 D E = 6 \sqrt {3}. $\)


∴MA+MB+MD 的最小值是 \(6 \sqrt { 3 }\)

故选：D．

9．如图，在矩形 ABCD 中，AB＝6，BC＝8，点 E 是 AD 边上的动点，点 M 是点 A 关于直线 BE 的对称点，连接 MD，则 MD 的最小值是（ ）

![](../images/0a3d1db25291a2ee17cddc86c0abb76e56e7f8cf5ca87af3eb12b118ad6aa197.jpg)

A．6

B．5

C．4

D．3

【解答】解：连接 BD，以点 B 为圆心，BA 为半径作圆，交 BD 于点 M，

∵四边形 ABCD 为矩形，


$\(\therefore \angle A = 90^{\circ}, $\)



$\(\therefore B D = \sqrt {A B^{2} + A D^{2}} = \sqrt {6^{2} + 8^{2}} = 10, $\)


∵点 A 和点 M 关于 BE 对称，


$\(\therefore A B = B M = 6, $\)



$\(\therefore D M = B D - B M = 10 - 6 = 4. $\)


故 DM 的最小值为 4

故选：C

![](../images/9634a80467da9a82ffbf567d59bc65dac482e8edc6228af98c2618421b42fd20.jpg)

10．如图，在矩形 ABCD 中，E，F 分别是边 AB，AD 上的动点，P 是线段 EF 的中点，\(PG \bot BC\)，\(PH \bot CD\)，G，H 为垂足，连接 GH．若 AB＝8，AD＝6，EF＝5，则 GH 的最小值是 7.5

![](../images/445e252aefa6701e7d9b3a88046d5efaa870bfc5a05ab3d5dfdfee623cc5fdb8.jpg)

【解答】解：连接 AC、AP、CP，如图所示：

∵四边形 ABCD 是矩形，


$\(\therefore B C = A D = 6, \angle B A D = \angle B = \angle C = 90^{\circ}, $\)



$\(\therefore A C = \sqrt {A B^{2} + B C^{2}} = \sqrt {8^{2} + 6^{2}} = 10, $\)


∵P 是线段 EF 的中点，


$\(\therefore A P = \frac {1}{2} E F = 2. 5, $\)


∵\(PG \bot BC\)，\(PH \bot CD\)，


$\(\therefore \angle P G C = \angle P H C = 90^{\circ}, $\)


∴四边形 PGCH 是矩形，


$\(\therefore G H = C P, $\)


当 A、P、C 三点共线时，CP 最小 \(= _ { A C \mathrm { ~ - ~ } A P = 10 \mathrm { ~ - ~ } 2 . 5 = 7 . 5 }\) ，

∴GH 的最小值是 7.5，

故答案为：7.5

![](../images/e0f96251ecd7ec012ed8b2d3813818cf2324be10083931a5893a8818d8204e5a.jpg)

11．如图，正方形 ABCD 的边长为 2，E 为与点 D 不重合的动点，以 DE 为一边作正方形 DEFG．设 DE＝

d1，点 F、G 与点 C 的距离分别为 d2、d3，则 \(d_{1} { + } d_{2} { + } d_{3}\) 的最小值为 \(2 { \sqrt { 2 } } .\)

![](../images/4f2964e47be9b46f024399096d30dcb67e37a8abde5a4bdd12df46f72af31562.jpg)

【解答】解：如图，连接 AC、AE、CF、CG，

![](../images/c5fa96a80887bf2f53a16b276f060fa88be65c0ac1cf0a097772d0704c829d1c.jpg)

在正方形 ABCD 和正方形 DEFG 中， \(\scriptstyle A D = C D, D E = D G = E F, \angle A D C = \angle E D G = 90^{\circ}\)


$\(\therefore \angle A D C - \angle E D C = \angle E D G - \angle E D C, $\)



$\(\therefore \angle A D E = \angle C D G, $\)



$\(\therefore \triangle A D E \cong \triangle C D G, $\)



$\(\therefore A E = C G, $\)



$\(\therefore d_{1} + d_{2} + d_{3} = D E + C F + C G = E F + C F + A E, $\)


∴当点 A、E、F、C 在同一直线上时（此时点 F 与点 C 重合），DE+CF+AE 最小，最小值为线段 AC 长，

![](../images/d0528a2ac53cd5c9c4ace7976a788cef6b66f49b9123772134e7e1a1f8ee0cfa.jpg)

在 \(\mathrm { R t } \triangle A B C\) 中， \(A C = \sqrt { A B^{2} + B C^{2} } = \sqrt { 2^{2} + 2^{2} } = 2 \sqrt { 2 }\) ，

\(\therefore d_{1} + d_{2} + d_{3}\) 的最小值为 \(2 \sqrt { 2 }\)

12．如图所示，在边长为 2 的菱形 ABCD 中， \(\angle D A B = 60^{\circ}\) °，点 E 为 AB 中点，点 F 是 AC 上一动点，则\(E F { + } B F\) 的最小值为 \(\phantom { }_{-} \sqrt { 3 }\) ．（提示：根据轴对称的性质）

![](../images/4a43d5cd21791ecac61ad749eeb2d2eefba05771a84b6ef58e774e5af6edced5.jpg)

【解答】解：连接 DB，DE，设 DE 交 AC 于 M，连接 MB，DF，

![](../images/cd3f15fb633bc6f045ab528f61e0b35037b91886244111819ad774c117952473.jpg)

∵四边形 ABCD 是菱形，

∴AC，BD 互相垂直平分，

∴点 B 关于 AC 的对称点为 D，


$\(\therefore F D = F B, $\)



$\(\therefore F E + F B = F E + F D \geqslant D E. $\)


只有当点 F 运动到点 M 时，取等号（两点之间线段最短），

\(\triangle A B D\) 中， \(\scriptstyle A D = A B, \angle D A B = 60^{\circ}\) ，

\(\therefore \triangle A B D\) 是等边三角形

\(\because E\) 为 AB 的中点，

∴ \(. D E \bot A B,\)


$\(\therefore A E = \frac {1}{2} A D = 1, D E = \sqrt {\mathrm{AD}^{2} - \mathrm{AE}^{2}} = \sqrt {2^{2} - 1^{2}} = \sqrt {3}, $\)


\(\therefore E F { + } B F\) 的最小值为 \({ \sqrt { 3 } } .\)

13．如图，P 是 \(\mathrm { R t } \triangle A B C\) 的斜边 AC（不与点 A、C 重合）上一动点，分别作 \(P M \bot A B\) 于点 \(M, ~ P N \bot B C\) 于点 N，O 是 MN 的中点，若 AB＝5，BC＝12，当点 P 在 AC 上运动时，BO 的最小值是 \(- \frac { 30 } { 13 } -\)

![](../images/76a01e80d60f3c469c2d5347dac0e100f629b4b97e7755a1ec7b21b2cf4d4958.jpg)

【解答】解：连接 BP，如图所示：

![](../images/66496db4652f931ab7564dbcac8687e998e33ab8861d66a311778240777b27e9.jpg)

\(\because \angle A B C = 90^{\circ}\) ，\(PM \bot AB\) 于点 M，\(PN \bot BC\) 于点 N，


$\(\therefore \angle A B C = \angle P M B = \angle P N B = 90^{\circ}, $\)


∴四边形 BMPN 是矩形， \(A C = \sqrt { A B^{2} + B C^{2} } = \sqrt { 5^{2} + 12^{2} } = 13\)

\(\therefore B P = M N, ~ B P\) 与 MN 互相平分，

∵点 O 是 MN 的中点，

∴点 O 是 BP 的中点，


$\(\therefore B O = \frac {1}{2} B P = \frac {1}{2} M N, $\)


当 \(B P \bot A C\) 时，BP 最小＝ \(= \frac { A B \cdot B C } { A C } = \frac { 5 \times 12 } { 13 } = \frac { 60 } { 13 },\)


$\(\therefore M N = \frac {60}{13}, $\)


\(\therefore B O\) 的最小值 \(= \frac { 1 } { 2 } M N = \frac { 30 } { 13 }\)

故答案为： \(\frac { 30 } { 13 }\)

14．如图，在边长为 2 的菱形 ABCD 中， \(\angle A = 60^{\circ}\) °，点 M 是 AD 边的中点，点 N 是菱形内一动点，且满足 \(M N = 1\) ，连接 CN，则 CN 的最小值为 \(- \sqrt { 7 } - 1\)

![](../images/acf6dcadda9bd88070ed5c5e2707675fd7e5306cd5af4ad918601b5836c67170.jpg)

【解答】解：过点 M 作 \(M H \bot C D\) ，交 CD 的延长线于点 H，如图所示：

![](../images/b1ebd53d6d9f6548f11affa53ee09379e3016ab63c02ac1c99abf9568140d96e.jpg)

在边长为 2 的菱形 ABCD 中， \(\angle A = 60^{\circ}, A B \parallel C D\) ，


$\(\therefore \angle H D M = \angle A = 60^{\circ}, $\)



$\(\therefore \angle H M D = 30^{\circ}, $\)


∵点 M 是 AD 边的中点，


$\(\therefore D M = 1, $\)



$\(\therefore D H = \frac {1}{2}, $\)


根据勾股定理，得 \(H M = \frac { \sqrt { 3 } } { 2 }\)


$\(\because C D = 2, $\)



$\(\therefore C H = \frac {5}{2}, $\)


根据勾股定理，得 \(C M = { \sqrt { 7 } }\) ，


$\(\because M N = 1, $\)


当点 N 运动到线段 CM 上的点 \(N^{\prime}\) ′时，CN 取得最小值，


$\(C N^{\prime} = C M - M N = \sqrt {7} - 1, $\)


\(\therefore C N\) 的最小值为 \(\sqrt { 7 } - 1\) ，

故答案为： \(\sqrt { 7 } - 1\) ．

15．如图，在菱形 ABCD 中，AC＝24，BD＝10．E 是 CD 边上一动点，过点 E 分别作 \(E F \bot O C\) 于点 F，EG\(\perp O D\) 于点 G，连接 FG，则 FG 的最小值为 \(- { \frac { 60 } { 13 } } .\)

![](../images/0a2bfa8bda200068b3fd86c4bda5f1f7cdd4d33e71f1259f9ab019b605e06c88.jpg)

【解答】解：连接 OE，作 \(O H \bot C D\) 于点 H，

∵四边形 ABCD 是菱形， \(A C = 24, B D = 10\) ，


$\(\therefore A C \perp B D, O C = O A = \frac {1}{2} A C = 12, O D = O B = \frac {1}{2} B D = 5, $\)



$\(\therefore \angle C O D = 90^{\circ}, $\)



$\(\therefore C D = \sqrt {O C^{2} + O D^{2}} = \sqrt {12^{2} + 5^{2}} = 13, $\)



$\(\because \frac {1}{2} C D \cdot O H = \frac {1}{2} O C \cdot O D = S_{\triangle C O D}, $\)



$\(\therefore \frac {1}{2} \times 13 O H = \frac {1}{2} \times 12 \times 5, $\)


解得 \(O H = \frac { 60 } { 13 }\)

\(\because E F \bot O C\) 于点 F，\(EG \bot OD\) 于点 G，


$\(\therefore \angle O F E = \angle O G E = \angle F O G = 90^{\circ}, $\)


∴四边形 OGEF 是矩形，


$\(\therefore O E = F G, $\)



$\(\therefore O E \geqslant O H, $\)



$\(\because F G \geqslant \frac {60}{13}, $\)


∴FG 的最小值为 \(\frac { 60 } { 13 }\)

故答案为： \(\frac { 60 } { 13 }\)

![](../images/7caec11d25a7331cf2927a03745b95edb78d63407bc2a448b7754ce77f7fff37.jpg)

16．如图，矩形 ABCD 中，AB＝6，AD＝4，点 E，F 分别是 AB，DC 上的动点，\(EF \parallel BC\)，则 BF+DE 最小值是（ ）

![](../images/46f8b051184df254416690e2eca356345db6ec8a2a003df16c7c1ee94124534e.jpg)

A．13

B．10

C．12

D．5

【解答】解：延长 AD，取点 M，使得 AD＝DM，连接 MP，如图，

![](../images/0ec23b2e96599110b13071ab0e563f7bf1946831c563842189b936565e0d3623.jpg)

∵ \(E F \parallel B C\) ，四边形 ABCD 是矩形，

∴四边形 AEFD 和四边形 EBCF 是矩形，


$\(\because A D = D M, A E = D F, \angle E A D = \angle F D M = 90^{\circ}, $\)



$\(\therefore \triangle A D E \cong \triangle D M F (S A S), $\)



$\(\therefore D E = M F, $\)



$\(\therefore B F + D E = B F + F M, $\)


∵点 E，F 分别是 AB，DC 上的动点，

故当 B，F，M 三点共线时， \(B F { + } D E\) 的值最小，且 \(B F { + } D E\) 的值等于 BM 的值，

在 Rt\(\triangle BAM\) 中， \(\mathrm { B M } = \sqrt { \mathrm { A B }^{2} + \mathrm { A M }^{2} } = \sqrt { 6^{2} + ( 2 \times 4 )^{2} } = 10 \cdot\)

故选：B．

17．如图，在边长为 4 的正方形 ABCD 中，点 E、F 分别是边 BC、CD 上的动点．且 BE＝CF，连接 BF、DE，则 BF+DE 的最小值为（ ）

![](../images/4b3c82e3a677122390dce39b78284be5c7fab80fbc23faa7cf42530b770d8575.jpg)

A． \(4 \sqrt { 3 }\)

B． \(2 \sqrt { 3 }\)

C． \(4 \sqrt { 5 }\)

D． \(2 \sqrt { 5 }\)

【解答】解：连接 AE，如图 1，

\(\because\) 四边形 ABCD 是正方形，


$\(\therefore A B = B C, \angle A B E = \angle B C F = 90^{\circ}. $\)


又 \(B E = C F,\)


$\(\therefore \triangle A B E \cong \triangle B C F (S A S). $\)



$\(\therefore A E = B F. $\)


![](../images/9cff53e9c1584468d6c2eb2239555b1a681e75ab53df373292f75ed341d53ac1.jpg)

图1

所以 BF+DE 最小值等于 AE+DE 最小值

作点 A 关于 BC 的对称点 H 点，如图 2，

连接 BH，则 A、B、H 三点共线，

连接 DH，DH 与 BC 的交点即为所求的 E 点

根据对称性可知 \(A E = H E, H A = 4 { + } 4 = 8\) ，

所以 \(A E { + } D E = D H\)

![](../images/3e3bfb3bd5fd86c9b32f1aaf632fc25548fa4bb7cc5ce2b7d73890a6578419b5.jpg)

在 \(\mathrm { R t } \triangle A D H\) 中， \(D H = \sqrt { \mathrm { A H }^{2} + \mathrm { A D }^{2} } = \sqrt { 8^{2} + 4^{2} } = 4 \sqrt { 5 }\)

\(\therefore B F { + } D E\) 最小值为 \(4 \sqrt { 5 }\)

故选：C

18．如图，在 \(\mathrm { R t } \triangle A B C\) 中， \(\angle B A C = 90^{\circ}\) ，且 AB＝6，AC＝8，点 D 是斜边 BC 上的一个动点，过点 D 分别作 \(D M \bot A B\) 于点 \(M, ~ D N \bot A C\) 于点 N，连接 MN，点 O 为 MN 的中点，则线段 AO 的最小值为（ ）

![](../images/06962f62c1f227e3bed922b3f234236a90bbcaf605d0d19335ae4a20c8a8b916.jpg)

A．4.8

B．5

C．2.4

D．3.6

【解答】解：如图，连接 AD，

![](../images/8fbcc21aa0557a68f1de565f381426d0e26166609d3544e69f91d2051b6e3412.jpg)

\(\because \angle B A C = 90^{\circ}\) ，且 \(A B = 6, A C = 8\) ，


$\(\therefore \mathrm{BC} = \sqrt {\mathrm{BA}^{2} + \mathrm{AC}^{2}} = \sqrt {6^{2} + 8^{2}} = 10, $\)


∵\(DM \bot AB\)，\(DN \bot AC\)，


$\(\therefore \angle D M A = \angle D N A = \angle B A C = 90^{\circ}, $\)


∴四边形 DMAN 是矩形，


$\(\therefore M N = A D, \quad A O = \frac {1}{2} A D, $\)


∴当 \(A D \perp B C\) 时，AD 的值最小，

此时， \(S_{\Delta A B C} = \frac { 1 } { 2 } A B \cdot A C = \frac { 1 } { 2 } B C \cdot A D\)


$\(\therefore \mathrm{AD} = \frac {\mathrm{AB} \cdot \mathrm{AC}}{\mathrm{BC}} = \frac {6 \times 8}{10} = 4. 8, $\)


∴AO 的最小值为 2.4，

故选：C

19．如图，在 \(\triangle A B C\) 中， \(\angle C = 90^{\circ}\) °，点 D 在斜边 AB 上，E、F 分别在直角边 CA、BC 上，且 \(D E \bot A C\) ，\(D F \parallel A C .\) ．

（1）求证：四边形 CEDF 是矩形；  
（2）连接 EF，若 C 到 AB 的距离是 5，求 EF 的最小值

![](../images/71f3092e3b87058dcf99456518029e3f15cbae5353babd6f0a3b7b9fbebd185b.jpg)

【解答】（1）证明： \(\because D F \parallel A C, \angle C = 90^{\circ}\)


$\(\therefore \angle D F B = \angle C = 90^{\circ}, $\)



$\(\therefore \angle D F C = 90^{\circ} = \angle C, $\)



$\(\because D E \perp A C, $\)



$\(\therefore \angle D E C = 90^{\circ} = \angle D F C = \angle C, $\)


∴四边形 CEDF 是矩形；

（2）解：连接 CD，如图所示：

由（1）可知，四边形 CEDF 是矩形，


$\(\therefore C D = E F, $\)


∴当 CD 有最小值时，EF 的值最小，

∵当 \(C D \bot A B\) 时，CD 有最小值，

\(\therefore C D \bot A B\) 时，EF 有最小值，

\(\because C\) 到 AB 的距离是 5，即点 C 到 AB 的垂直距离为 5，

\(\therefore C D\) 的最小值为 5，

\(\therefore E F\) 的最小值为 5

![](../images/ad15c437ede6e538e7c6a030bf4d340fd6e6f785339173611eca3ffa02782b62.jpg)

20．如图所示，在菱形 ABCD 中， \(\angle A B = 8, \angle B A D = 120^{\circ}\) ， \(\triangle A E F\) 为等边三角形，点 E、F 分别在菱形的边 BC、CD 上滑动，且 E、F 不与 B、C、D 重合

（1）证明不论 E、F 在 BC、CD 上如何滑动，总有 \(B E = C F\)

（2）当点 E、F 在 BC、CD 上滑动时，分别探讨四边形 AECF 和 \(\triangle C E F\) 的面积是否发生变化？如果不变，求出这个定值；如果变化，求出最大（或最小）值

![](../images/b14b2e12b40ca435cf1a25151ead024ef147b709b8b31ae95b2a642f7633a311.jpg)

【解答】（1）证明：连接 AC，如图所示：

![](../images/7aaf61e8a3b82981e5b6f78a53dd53f33eb305eacdcfbda91ad607941edfbc23.jpg)

∵四边形 ABCD 为菱形， \(\angle B A D = 120^{\circ}\)


$\(\therefore \angle B A C = 60^{\circ}, $\)


∵ \(\triangle A E F\) 是等边三角形，


$\(\therefore \angle E A F = 60^{\circ}, $\)



$\(\therefore \angle B A E + \angle E A C = 60^{\circ}, \angle C A F + \angle E A C = 60^{\circ}, $\)



$\(\therefore \angle B A E = \angle C A F, $\)


∵四边形 ABCD 为菱形，


$\(\therefore A D \parallel B C, $\)



$\(\therefore \angle A B C + \angle B A D = 180^{\circ}, $\)



$\(\because \angle B A D = 120^{\circ}, $\)



$\(\therefore \angle A B C = 60^{\circ}, $\)


∴\(\triangle ABC\) 和\(\triangle ACD\) 为等边三角形，


$\(\therefore \angle A C F = 60^{\circ}, A C = A B, $\)


在\(\triangle ABE\) 和 \(\triangle A C F\) 中，


$\(\left\{ \begin{array}{l} \angle B A E = \angle C A F \\ A B = A C \\ \angle A B C = \angle A C F \end{array}, \right. $\)



$\(\therefore \triangle A B E \cong \triangle A C F (A S A). $\)



$\(\therefore B E = C F. $\)


（2）解：四边形 AECF 的面积不变

理由：由（1）得 \(\triangle A B E { \cong } \triangle A C F\) ，

则 \(S_{\triangle} A B E = S_{\triangle} A C F\) ，

故 \(S_{\perp \perp \perp \perp} A E C F = S_{\triangle} A E C { + } S_{\triangle} A C F = S_{\triangle}\)

\(A E C + S_{\triangle} A B E = S_{\triangle} A B C\) ，是定值；

作 \(A H \bot B C\) 于 H 点，如图所示：

![](../images/12435b0daa412d3d2415201c7266df3fd888e62a7b46e288f40510d3f83c79c6.jpg)


$\(\because \angle A H B = 90^{\circ}, \angle A B C = 60^{\circ}, $\)



$\(\therefore \angle B A H = 90^{\circ} - 60^{\circ} = 30^{\circ}, $\)



$\(\therefore \mathrm{BH} = \frac {1}{2} \mathrm{AB} = 4, $\)


在 \(\mathrm { R t } \triangle A B H\) 中，根据勾股定理得： \(A H = \sqrt { A B^{2} - B H^{2} } = \sqrt { 8^{2} - 4^{2} } = 4 \sqrt { 3 }\) ，


$\(\therefore S _ {\text {四边形}} A E C F = S_{\triangle} A B C = \frac {1}{2} \cdot B C \cdot A H = 16 \sqrt {3}. $\)



$\(\because S_{\triangle} C E F = S _ {\text {四边形}} A E C F - S_{\triangle} A E F = \frac {1}{2} S _ {\text {菱形}} A B C D - S_{\triangle} A E F, $\)


\(\therefore \triangle C E F\) 的面积随 \(\triangle A E F\) 面积的变化而变化，

\(\because \triangle A E F\) 为等边三角形，

∴当 AE 最短时， \(\triangle A E F\) 的面积最小，则 \(\triangle C E F\) 的面积有最大值，

∵当 \(A E \bot B C\) 时，AE 最小，

\(\therefore A E\) 的最小值为 AH 的长 \(4 { \sqrt { 3 } },\) ，

过点 A 作 \(A M \bot E F\) ，垂足为 M，如图所示：

![](../images/5226046844aa947246700fed059f6740286100375ccc8edf6a48fb6acae5c46f.jpg)

\(\because \triangle A E F\) 为等边三角形，


$\(\therefore \mathrm{AF} = \mathrm{EF} = \mathrm{AE} = 4 \sqrt {3}, \angle A E F = 60^{\circ}, $\)



$\(\therefore E M = M F = \frac {1}{2} E F = 2 \sqrt {3}, $\)



$\(\therefore \mathrm{AM} = \sqrt {\mathrm{AE}^{2} - \mathrm{EM}^{2}} = \sqrt {(4 \sqrt {3})^{2} - (2 \sqrt {3})^{2}} = 6, $\)



$\(\therefore S_{\triangle A E F} = \frac {1}{2} \times 4 \sqrt {3} \times 6 = 12 \sqrt {3}, $\)



$\(\therefore S_{\triangle C E F} = S _ {\text {四边形} A E C F} - S_{\triangle A E F} = 16 \sqrt {3} - 12 \sqrt {3} = 4 \sqrt {3}, $\)


即\(\triangle CEF\) 的面积的最大值为 \(4 { \sqrt { 3 } } .\)

21．如图①，四边形 ABCD 是正方形， \(\triangle A B E\) 是等边三角形，M 为对角线 BD（不含 B 点）上任意一点，将 BM 绕点 B 逆时针旋转 \(60^{\circ}\) 得到 BN，连接 EN、AM、CM

（1）连接 MN， \(\triangle B M N\) 是等边三角形吗？为什么？  
（2）求证： \(\triangle A M B \cong \triangle E N B\) ；  
（3）①当 M点在何处时，AM+CM 的值最小；  
②如图②，当 M 点在何处时，AM+BM+CM 的值最小，请你画出图形，并说明理由

![](../images/71b08b5f53b08c657ac63ca78458b27557a6cc8f70ae8fd70e3f5edebc474047.jpg)

![](../images/4408d55b8246db54c7a90ac2479dbd4ed023a869fa4a052d62950acee6e0b026.jpg)

图②

【解答】（1）解：\(\triangle BMN\) 是等边三角形

理由如下：如图①，∵BM 绕点 B 逆时针旋转 \(60^{\circ}\) 得到 BN，


$\(\therefore B M = B N, \angle M B N = 60^{\circ}, $\)


∴\(\triangle BMN\) 是等边三角形；

（2）证明： \(\because \triangle A B E\) 和 \(\triangle B M N\) 都是等边三角形，


$\(\therefore A B = E B, \quad B M = B N, \quad \angle A B E = \angle M B N = 60^{\circ}, $\)



$\(\therefore \angle A B E - \angle A B N = \angle M B N - \angle A B N, $\)


即 \(\angle A B M = \angle E B N,\)

在 \(\triangle A M B\) 和 \(1 \triangle E N B\) 中，


$\(\left\{ \begin{array}{l} A B = E B \\ \angle A B M = \angle E B N, \\ B M = B N \end{array} \right. $\)



$\(\therefore \triangle A M B \cong \triangle E N B (S A S); $\)


（3）①由两点之间线段最短可知 A、M、C 三点共线时，AM+CM 的值最小，

∵四边形 ABCD 是正方形，

∴点 M 为 BD 的中点；

②当点 M 在 CE 与 BD 的交点时， \(A M { + } B M { + } C M\) 的值最小，

理由如下：如图②， \(\because \triangle A M B \cong \triangle E N B,\)


$\(\therefore A M = E N, $\)


∵\(\triangle BMN\) 是等边三角形，


$\(\therefore B M = M N, $\)



$\(\therefore A M + B M + C M = E N + M N + C M, $\)


由两点之间线段最短可知，点 E、N、M、C 在同一直线上时，EN+MN+CM，故，点 M 在 CE 与 BD 的交点时，AM+BM+CM 的值最小

![](../images/4c875db3ff55a68756258dabfa70f8229f2fc06155501e2b0fd25cdcc4e889e4.jpg)

图②

![](../images/0b54a15366e57a2ed000ff0a7245e5ef070197ce8aa3f669db81057181617fe0.jpg)

图①
