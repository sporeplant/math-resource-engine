---
name: html-to-seewo
description: 将 HTML 课件转为希沃白板可导入包（JSON + 逐页 PNG 截图）
---
...
**输入**：HTML 课件文件路径
**输出**：在 HTML 同级目录生成 `{HTML主名}.json` + `assets/slide_01.png` ~ `slide_N.png`，可通过 `--output-dir` 指定输出目录
...
- 图片存入 `assets/` 子目录
- 生成 MrePlugin 格式的 JSON（与 HTML 文件同名）
...
遵循 `support/easinote/MRE-Plugin/` 的 MrePlugin 规范，每页引用一张对应 PNG 图片。文件名与源 HTML 文件同名（如 `13讲_统计调查与直方图复习_课件.json`）。
...
- [ ] JSON 文件存在且为合法 JSON（文件名与 HTML 同名）
- [ ] `assets/` 目录存在
...
- 禁止修改 JSON 的输出路径和文件名，必须与 HTML 同级且同名，最终归入 `exports/`
...
> 📦 希沃导入包已生成：
> - `{目录}/exports/{HTML主名}.json`
> - `{目录}/exports/assets/slide_01.png` ~ `slide_N.png`
>
> 导入方式：希沃白板5 → 打开或新建课件 → 右键空白页 → 插件菜单 → MRE导入 → 选择 `exports/` 下的 `.json` 文件
