...
- [ ] 已读取 `orchestrator/resource-scheduling.md`
- [ ] 已完成教材-练习册双资源盘点，明确当堂检测、课后作业和后移题去向
...
#### 第一层：传统教学设计正文
- [ ] 是否依次包含"教学内容、教学目标、教学重点与难点、教学准备、教学过程、当堂检测、课后作业、板书设计、设计依据简记"
- [ ] 是否未泄漏 LO-*、AS-*、ACT-*、ASK_* 等后台字段
- [ ] 当堂检测是否写明检测目标、限时和教师可读题目来源
- [ ] 课后作业是否分基础层必做、中间层必做、拓展层选做，并标注预计用时
- [ ] 是否说明后移到习题课、讲评课、周练或单元整理的题目安排
- [ ] 板书设计是否使用普通 Markdown 排版，未使用代码块

#### 第二层：完整结构化设计
- [ ] 是否使用 `<details>` 折叠块
- [ ] 是否包含 meta、knowledge_analysis、objectives、assessment、problem_chain、lesson_flow、resource_audit、practice、homework、deferred_exercises、boardwork、consistency_matrix、quality_check
- [ ] 稳定 ID、题源字段、评价证据和审核信息是否完整保留

#### 双层一致性
- [ ] 第一层教学过程与第二层 ACT-* 活动是否数量、顺序和含义一致
- [ ] 两层活动时间之和是否一致且不超过 40 分钟
- [ ] 第一层当堂检测、课后作业和后移题都能在第二层找到对应任务和题源
